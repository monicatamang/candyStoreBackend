...
def edit_candy():
    ...
    old_candy = dbhelpers.run_select_statement("SELECT c.name, c.description, c.price_in_dollars, c.image_url, c.user_id, c.id, u.username FROM users u INNER JOIN candy c ON u.id = c.user_id WHERE u.id = ? AND c.id = ?", [user_id, candy_id])
    ...
    row_count = dbhelpers.run_update_statement("UPDATE candy SET name = ?, description = ?, price_in_dollars = ?, image_url = ? WHERE user_id = ? AND id = ?", [candy_name, candy_description, candy_price, candy_image, user_id, candy_id])

    # If the user's candy post is updated, get the updated candy post from the database
    if(row_count == 1):
        edited_candy = dbhelpers.run_select_statement("SELECT c.name, c.description, c.price_in_dollars, c.image_url, c.user_id, c.id, u.username FROM users u INNER JOIN candy c WHERE u.id = ? AND c.id = ?", [user_id, candy_id])
